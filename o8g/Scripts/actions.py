...
if os.getenv('ENV') == 'devel':
    from o8g.Scripts.v2.me import my_deck, my_play_hand
    from o8g.Scripts.v2.card import is_outfit
...
def nextPhase(group=table, x=0, y=0):
    ...
    if getGlobalVariable('Shootout') == 'True':
        if not confirm("There's still a shootout ongoing. Do you want to end it and move to the next phase?"):
            return
        else:
            goToShootout()
    if shared.Phase == 4:
        ...
        shared.Phase += 1  # Otherwise, just move up one phase
    if shared.Phase == 1:
        goToGamblin()
    elif shared.Phase == 2:
        goToUpkeep()
...
def goToShootout(group=table, x=0, y=0):  # Start or End a Shootout Phase
    if getGlobalVariable(
            'Shootout') == 'False':  # The shootout phase just shows a nice notification when it starts and does nothing else.
...
def defaultAction(card, x=0, y=0):
    ...
    mute()
    side = None
    calledOut = getGlobalVariable(
        'Called Out')  # We grab this global variable so that we don't have to check it multiple times and delay them game
    if card.model == 'c421c742-c920-4cad-9f72-032c3378191e':  # If the player has double-clicked the lowball card, we assume they are the ones that won lowball.
        ...
        if card.Type == 'Dude' and not card.highlight:
            for c in table:
                if c.highlight == AttackColor:
                    if c.controller == me:
                        ...
                        side = 'Defence'  # If we found out opponent's dude as attacker and there's only 2 player in the game, we assume we're the defender.
            ...
            if not side:  # If we still couldn't determine which side the player is on, we just ask directly.
                if confirm(
                    "Sorry, but I could not automatically determine if you're the attacking or defending player.\n\nIs this dude joining the attacking posse?"):
                    side = 'Attack'
                else:
                    side = 'Defence'
            if side == 'Defence':
            ...
            boot(card)
    elif len([c for c in table if
              c.Type == 'Dude' and c.targetedBy and c.targetedBy == me and c.controller != me]) and calledOut == 'None':  # If the player has an opposing dude targeted when they doubled clicked, we assume they want to call out
        callout(card)
    elif calledOut != 'None' and Card(num(
            calledOut)) == card:  # if there is a callout in progress and we just double clicked the callout's target, we assume they want to accwept it.
        acceptCallout()
    else:
        boot(card)
    debugNotify("<<< defaultAction()")  # Debug
...
def get_outfits(group=None):
    if group is None:
        group = my_cards()
    return (build(card) for card in group if is_outfit(card))


def get_dudes(group):
    if group is None:
        group = my_cards()
    return (build(card) for card in group if is_dude(card))
...
def setup(group, x=0, y=0):
    ...
    starting_dudes = [card for card in play_hand if is_dude(card)]
...
def pay(card):
    ...
    if me.GhostRock < cost:
        if not confirm("You do not seem to have enough Ghost Rock in your bank to play this card. "
                       "Are you sure you want to proceed? (If you do, your GR will go to the negative. "
    ...
    notify("{} has paid {} Ghost Rock. {} is left their bank".format(me, cost, me.GhostRock))
...
def ace(card, x=0, y=0):  # Ace a card. I.e. kill it and send it to the boot hill (i.e.graveyard)
    ...
    if card.highlight != DrawHandColor:  # We don't want to do anything else except move cards when they're not really in play.
        ...
        if card.markers[WantedMarker] == 1: notify("{} was wanted! Don't forget your bounty.".format(
            card))  # Remind the player to take a bounty for wanted dudes.
...
def discard(card, x=0, y=0):  # Discard a card.
    ...
    if (card.highlight == EventColor or card.Type == 'Joker') and re.search('Ace this card',
                                                                            card.Text):  # If the card being discarded was a joker or an event in a lowball hand
        ...
        if card.Type == 'Joker':
            ...
            notify("{} was the active event and has been aced as per card instructions.".format(card))
    else:
...
def upkeep(group, x=0, y=0):  # Automatically receive production and pay upkeep costs
    ...
    for card in cards:  # For each card...
        ...
        if gr - (2 * card.markers[
            JailbreakMarker]) > 0:  # If we still have any production left after removing jailbroken penalties,
    ...
    notify("{} has produced {} ghost rock this turn: {}".format(me, prod,
                                                                concat_prod))  # Inform the players how much they produced and from where.
    ...
    if upk > 0:  # If we need to pay any upkeep, we do it after receiving production for the turn.
        if me.GhostRock < upk:  # If we cannot pay with the money we have, then let the player decide what to do.
            ...
            notify(
                "{} has {}GR in their bank but needs to pay {}GR for upkeep ({}). No GR has been taken but please discard enough cards with upkeep and reduce your remaining Ghost Rock manually.".format(
                    me, me.GhostRock, upk, concat_upk))
        else:  # If we can pay the upkeep, do so.
            notify("{} has paid {} upkeed in total this turn. {}".format(me, upk,
                                                                         concat_upk))  # Inform the players how much they paid and for what.
...
def HNActivate(card, x=0, y=0):  # A function to add or remove High Noon (HN) markers.
    ...
    if card.markers[HNActivatedMarker] == 0:  # If we have no HN markers, add one
        ...
        notify(
            "{} Wanted to use {}'s High Noon ability but it has already been used this turn. Did they use a card effect?".format(
                me, card))
        return False
    return True
...
def NightfallUnboot(group, x=0,
                    y=0):  # This function simply runs all the cards the player controls through the nigtfall function.
    mute()
...
def inspectCard(card, x=0,
                y=0):  # This function shows the player the card text, to allow for easy reading until High Quality scans are procured.
...
def reCalculate(group=table, x=0, y=0, notification='loud'):
    ...
    cards = (card for card in table  # We only care for cards we control.
             if card.controller == me)
    for card in cards:
        count = num(card.Influence) + card.markers[InfluencePlusMarker] - card.markers[
            InfluenceMinusMarker]  # Put the card's total influence on a temp marker.
        if count > 0:  # We only care to do anything if the card had any influence
            ...
            concat_inf += '{} from {}'.format(str(count),
                                              card)  # Add the count as a string to the concatenated list before the name, e.g. "3 from Black Jack"
        ...
        count = num(card.Control) + card.markers[ControlPlusMarker] - card.markers[
            ControlMinusMarker]  # Put the card's total influence on a temp marker.
    ...
    if notification == 'loud':  # We only want to say the result if we're not explicitly asked to be silent (i.e. from the table action)
        notify(
            '{} has recalculated and found their influence to to be {}{}, and their Control Points to be {}{}.'.format(
                me, influence, concat_inf, control, concat_cp))
...
def plusControl(card, x=0, y=0, notification='loud', count=1):  # Adds an extra control marker to cards (usually deeds)
    ...
    if notification == loud:
        notify(
            "{} marks that {} provides {}  more control (Their total has been adjusted accordingly).".format(me, card,
                                                                                                             count))
...
def minusControl(card, x=0, y=0, notification='loud', count=1):  # Similar to adding Control but we remove instead.
    mute()
    if notification == loud:
        notify("{} marks that {} provides {} less control (Their total has been adjusted accordingly).".format(me, card,
                                                                                                               count))
    for i in range(0, count):
        if ControlPlusMarker in card.markers:
            ...
            if num(card.Control) - card.markers[ControlMinusMarker] > 0: modControl(
                -1)  # We only reduce a players totals if the control was above 0
...
def plusInfluence(card, x=0, y=0, notification='loud', count=1):  # The same as pluControl but for influence
    ...
    if notification == loud:
        notify(
            "{} marks that {}'s influence has increased by {} ({}'s total has been adjusted automatically)".format(me,
                                                                                                                   card,
                                                                                                                   count,
                                                                                                                   me))
    for i in range(0, count):
...
def minusInfluence(card, x=0, y=0, notification='loud', count=1):  # The same as minusContorl but for influence
    ...
    if notification == loud:
        notify(
            "{} marks that {}'s influence has decreased by {} ({}'s total has been adjusted automatically).".format(me,
                                                                                                                    card,
                                                                                                                    count,
                                                                                                                    me))
    for i in range(0, count):
...
def plusProd(card, x=0,
             y=0):  # Very much like plus Influence and control, but we don't have to worry about modifying the player's totals
    mute()
    if ProdPlusMarker in card.markers or ProdMinusMarker in card.markers:  # Putting the clarification about upkeep
        ...
        notify(
            "{} marks that {}'s production has increased by 1 GR (This will be automatically taken into account during upkeep).".format(
                me, card))
...
def minusProd(card, x=0, y=0):
    ...
    if ProdPlusMarker in card.markers or ProdMinusMarker in card.markers:
        ...
        notify(
            "{} marks that {}'s production has decreased by 1 GR (This will be automatically taken into account during upkeep).".format(
                me, card))
...
def plusValue(card, x=0, y=0, notification='loud', valuemod=None):
    ...
    if valuemod is None:
        valuemod = askInteger(
            "Increase {}'s value by how much? (Current value is: {})".format(card.name, calcValue(card)), 3)
        if valuemod is None:
            return
    ...
    if notification == loud:
        notify(
            "{} marks that {}'s value has increased by {} and is now {}.".format(me, card, valuemod, calcValue(card)))
...
def minusValue(card, x=0, y=0, notification='loud', valuemod=None):
    ...
    if valuemod is None:
        valuemod = askInteger(
            "Decrease {}'s value by how much? (Current value is: {})".format(card.name, calcValue(card)), 3)
        if valuemod is None:
            return
    ...
    if notification == loud:
        notify(
            "{} marks that {}'s value has decreased by {} and is now {}.".format(me, card, valuemod, calcValue(card)))


def setValue(card, x=0, y=0):
    mute()
    current_value = calcValue(card, 'raw')
    new_value = askInteger("What should the new value be? (use direct number format: 1 - 13)",
                           calcValue(card, 'numeral'))
...
def modWantedMarker(card, x=0,
                    y=0):  # Similar to the doesNotUnboot function but with markers. Adds or removes the wanted marker from a dude.
    mute()
    if card.Type == "Dude":
...
def addHarrowedMarker(card, x=0,
                      y=0):  # Same as the modWantedMarker but you cannot remove it. You get a notification instead.
    mute()
    if card.Type == "Dude":
...
def callout(card, x=0, y=0):  # Notifies that this dude is calling someone out.
    mute()
    if getGlobalVariable('Called Out') != 'None' and not confirm(
        ":::WARNING::: There seems to be another call out in progress. Override it?"): return
    if card.Type == "Dude":
        targetDudes = [c for c in table if
                       c.Type == 'Dude' and c.targetedBy and c.targetedBy == me and c.controller != me]
        ...
        setGlobalVariable('Called Out', str(
            targetD._id))  # We store the called out dude as a global variable so that the owner can easier select their answer.
        if card.orientation == Rot90: notify(
            ":::WARNING::: Remember that you need a card effect to call out someone while booted)".format(card))
        card.highlight = AttackColor
    else:
        whisper(":::ERROR::: You can only initiate a call-out with a dude")
...
def goods(card, x=0,
          y=0):  # Notifies that this dude is about to receive some goods, either by trading or by playing from your hand
    ...
    if card.Type == "Dude":
        ...
        if card.orientation == Rot90: notify(
            "(Remember that you need a card effect to receive goods while booted)".format(card))
        ...
        if AttachingCard not in AttachedCards:
            AttachedCards[
                AttachingCard] = 1  # We set the dictionary AttachedCards to store an entry for the card which is about to receive goods, so as not to crash our scripts later on.
...
def tradeGoods(card, x=0, y=0):
    ...
    if not newHost:
        ...
        return
    chkHighNoon()
    if newHost.orientation != Rot0 and not confirm(
        "You can only trade goods to unbooted dudes. Bypass restriction?"): return
    hostCards = eval(getGlobalVariable('Host Cards'))
    attachedGoods = [Card(att_id) for att_id in hostCards if
                     hostCards[att_id] == card._id and Card(att_id).Type == 'Good']
    chosenGoods = []
    if len(attachedGoods) == 0:
        whisper(":::ERROR::: This dude does not hold any goods they can trade.")
        return
    elif len(attachedGoods) == 1:
        attachCard(attachedGoods[0],
                   newHost)  # If there's only 1 goods attached, we assume that's the one that is going to be moved.
...
def acceptCallout(ignored=None, x=0, y=0):  # Same as the defending posse but with diferent notification.
    mute()
    if getGlobalVariable('Called Out') == 'None':
        whisper(":::ERROR::: There seems to be no callout in progress")
    else:
...
def refuseCallout(ignored=None, x=0,
                  y=0):  # Boots the dude and moves him to your home or informs you if they cannot refuse.
    # chooseSide()
    mute()
    if getGlobalVariable('Called Out') == 'None':
        whisper(":::ERROR::: There seems to be no callout in progress")
        ...
        if chickenDude.orientation == Rot90 and not confirm(
                ":::WARNING::: Normally booted dudes cannot refuse callouts. Bypass restriction and refuse anyway?"):
            ...
            if playeraxis == Xaxis:
                chickenDude.moveToTable(homeDistance(chickenDude) + (playerside * cwidth(chickenDude, -4)),
                                        0)  # ...and move them where we expect the player's home to be.
            elif playeraxis == Yaxis:
                chickenDude.moveToTable(0, homeDistance(chickenDude) + (playerside * cheight(chickenDude, -4)))
...
def playcard(card, retainPos=False):
    ...
    uniquecards = (tablecard for tablecard in table
                   # Lets gather all the cards from the table that may prevent us from playing our card
                   ...
                   and (tablecard.Type == 'Dude'  # But only dude or deeds...
                        ...
                        or (re.search('Unique.',
                                      tablecard.Text)  # ...or cards with an explicit "Unique" in the text that are Goods, Improvements or Spells.
    ...
    for player in players:
        acedcards = (acedcard for acedcard in player.piles['Boot Hill']
                     # Go through each player's Boot Hill looking for matches
    ...
    for chkcard in chkcards:  # Now we check the combined list to see if anything will block us from playing our card from the hand.
        if ((chkcard.controller == me and  # First lets see if this is an experienced version that we can play for free.
            ...
            if confirm(
                    "You seem to have another version of {} in play. Do you want to replace it with the version in your hand".format(
                            card.name)):
              ...
              re.search('There is no limit', chkcard.Text)):
            continue
        # We can still play our own non-unique cards that other players have in play.
            ...
            if chkcard.group == table:
                notify("{} wanted to bring {} in play but it is already on the table and owned by {}".format(me, card,
                                                                                                             chkcard.owner))
            else:  # if they're not on the table, they're in someone's boothill
                notify("{} wanted to bring {} in play but it currently RIP in {}'s Boot Hill".format(me, card,
                                                                                                     chkcard.owner))
            return
    if card.Type == "Dude":
        chkHighNoon()
        ...
        if card.Type == "Improvement":
            hostCard = findHost('Improvement')
        else:
            hostCard = findHost('Good')
        if not hostCard:
            if card.Type == "Improvement":
                whisper("You need to target the deed which is going to be improved")
            else:
                whisper("You need to target the dude which is going to purchase the goods")
            if retainPos: card.moveTo(me.piles['Play Hand'])
            return
        else:
            if hostCard.orientation != Rot0 and card.Type != "Improvement" and not confirm(
                "You can only attach goods to unbooted dudes. Bypass restriction?"): return
            if payCost(card.Cost, loud) == 'ABORT': return  # Check if the player can pay the cost. If not, abort.
            ...
            if re.search('Gadget', card.Text):
                if confirm("You are trying to create a gadget. Would you like to do a gadget pull at this point?"):
                    ...
                    notify("{} attempted to manufacture a {} and pulled a {} {}".format(hostCard, card,
                                                                                        fullrank(gadgetPull[0]),
                                                                                        fullsuit(gadgetPull[1])))
                else:
                    notify("{} has created a {} without a gadget pull.".format(hostCard, card))
            elif card.Type == "Spell":
                notify("{} has learned {}.".format(hostCard, card))
            elif card.Type == "Improvement":
                notify("{} has improved {} with {}.".format(me, hostCard, card))
            else:
                notify("{} has purchased {}.".format(hostCard, card))
    else:
        if payCost(card.Cost, loud) == 'ABORT': return  # Check if the player can pay the cost. If not, abort.
...
def pull(group=None, x=0, y=0,
         silent=False):  # Draws one card from the deck into the discard pile and announces its value.
    mute()
    if group is None:
        group = me.Deck
    Deck = me.Deck
    ...
    if not silent: notify(
        "{} Pulled a {} {}.".format(me, fullrank(rank), fullsuit(suit)))  # Announce them nicely to everyone.
    return (rank, suit)


def drawMany(group, count=None,
             notification='loud'):  # This function draws a variable number cards into the player's hand.
    mute()
    if count == None: count = askInteger("Draw how many cards to your Play Hand?",
                                         5)  # Ask the player how many cards they want.
    for i in range(0, count):
    ...
    if notification == loud: notify(
        "{} draws {} cards to their play hand.".format(me, count))  # And if we're "loud", notify what happened.


def setHandSize(
        group):  # A function to modify a player's hand size. This is used during nighfall when refilling the player's hand automatically.
    global handsize
...
def refill(group=None):  # Refill the player's hand to its hand size.
    ...
    if playhand < handsize:
        ...
        drawMany(me.Deck, handsize - playhand, silent)
...
def drawhandMany(group, count=None,
                 notification='loud'):  # Same as drawMany, but puts the cards into the player's Draw Hand pile.
    mute()
...
def harrow(card):  # Returns the top dude card from boot hill, into the table with a harrowed marker.
    ...
    if card.Type == 'Dude':
        ...
        if not re.search(r'\bHarrowed\b\.', card.Text):
            ...
            notify("{} has once again crawled out of a shallow grave.".format(card))
...
def revealHand(group=None, type='lowball', event=None):
    ...
    for card in group:  # For each card in the Draw Hand pile...
        ...
        if type == 'shootout':
            if card.name == "Fool's Joker":  # Check if the card is a joker that is invalid for shootouts. If so, replace it
                card.moveTo(me.piles['Discard Pile'])
                notify(
                    "A {} was revealed in {}'s shootout hand and has been discarded and replaced with a card from the top of the deck".format(
                        card, me))
            ...
            if playeraxis == Xaxis:
                card.moveToTable(homeDistance(card) - cardDistance(card) * 3 + i * (cwidth(card) / 4),
                                 cheight(card) * 2)
            elif playeraxis == Yaxis:
                card.moveToTable(cwidth(card) / -2 + i * (cwidth(card) / 4), homeDistance(card) - cardDistance(card))
            else:
                card.moveToTable(i * (cwidth(card) / 4) - cwidth(card),
                                 0)  # If the player is not on any side, put the cards in the middle.
            ...
            if card.name == "Death's Head Joker" or card.name == "Fool's Joker":  # Same as in shootouts above, but we don't want Death's Heads either
                if card.name == "Death's Head Joker":
                    card.moveTo(me.piles['Boot Hill'])  # Death's Head Jokers are aced
                else:
                    card.moveTo(me.piles['Discard Pile'])  # Fool's Jokers are discarded
                notify(
                    "A {} was revealed in {}'s lowball hand and has been replaced with a card from the top of the deck".format(
                        card, me))
                if len(me.Deck) == 0: reshuffle()
            ...
            if playeraxis == Xaxis:
                ...
                card.moveToTable(i * (cwidth(card) / 4) - cwidth(card), 0)
    ...
    if type == 'shootout':  # Finally, inform the players on what the hand is.
        notify(
            "{}'s Shootout hand is {}{} ({} {}, {} {}, {} {}, {} {}, {} {}). ".format(me, PokerHand(rank, suit, type),
                                                                                      cheatinchk(rank, suit),
                                                                                      fullrank(rank[0]),
                                                                                      fullsuit(suit[0]),
                                                                                      fullrank(rank[1]),
                                                                                      fullsuit(suit[1]),
        ...
        notify("{}'s Lowball Hand is {}{} ({} {}, {} {}, {} {}, {} {}, {} {}). ".format(me, PokerHand(rank, suit, type),
                                                                                        cheatinchk(rank, suit),
                                                                                        fullrank(rank[0]),
                                                                                        fullsuit(suit[0]),
                                                                                        fullrank(rank[1]),
...
def revealShootoutHand(group):
    ...
    for player in players:
        ...
        if player.HandRank < me.HandRank:
            notify("The winner is {} by {} ranks and {} must ace as many of their dudes in this shootout".format(me, (
                        me.HandRank - player.HandRank), player))
            clearHandRanks()
        elif player.HandRank > me.HandRank:
            notify("The winner is {} by {} ranks and {} must ace as many of their dudes in this shootout".format(player,
                                                                                                                 (
                                                                                                                             player.HandRank - me.HandRank),
                                                                                                                 me))
            clearHandRanks()
        else:
...
def revealLowballHand(group=None, type='normal'):
    ...
    if evCount > 1:  # If we have more than one, select one at random
        ...
        revealHand(group, 'lowball', foundEvents[
            eventPointer])  # Then pass its name to the next function so that it can be highlighted and announced.
...
def revealHandAsk(group):
    if confirm("Are you revealing a shootout hand?"):
        revealShootoutHand(group)
    else:
        revealLowballHand(group)
...
def playLowball(group=None):
    ...
    try:
        ...
        winLowball(
            winner=winner)  # If it's a python exception it means we tried to compare a string to a player object, therefore we actually have a player object in our 'winner' variable
...
def winLowball(group=None, x=0, y=0,
               winner=me):  # A function which sets the lowball winner and awards him the lowball money
    mute()
    if group is None:
        group = table
    ...
    notify("{} is the lowball winner has received {} Ghost Rock from the pot".format(winner, potCard.markers[
        mdict['Ante']]))  # Notify all other players
